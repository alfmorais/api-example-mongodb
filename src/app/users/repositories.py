...
class UserRepository:
    def create(self, db: Database, user: UserModel) -> str:
        ...
        result = db[settings.mongo_database].users.insert_one(_user)
        return str(result.inserted_id)

    def get(self, db: Database, document: str) -> UserModel:
        _filter = {"document": document}
        user = db[settings.mongo_database].users.find_one(_filter)
        if user:
            return UserModel(**user)

    def update(self, db: Database, document: str, user: UserModel) -> bool:
        result = db[settings.mongo_database].users.update_one(
            {"document": document},
    ...
    def delete(self, db: Database, document: str) -> bool:
        _filter = {"document": document}
        result = db[settings.mongo_database].users.delete_one(_filter)
